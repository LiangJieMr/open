...
gulp.task("clean:css", function() {
	return gulp.src(devUrl + "css191111/*.css")
		.pipe(gp.clean())
});
//清除html
...
gulp.task('sass', function() {
	return gulp.src("./src/sass/*.scss")
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "css191111"))
});

//静态等资源移动任务:图片、视频、字体等
gulp.task("assets11Move", function() {
	return gulp.src("./src/assets191111/**")
		.pipe(gp.changed(devUrl + "assets191111"))
		.pipe(gp.debug({
			title: '静态资源移动:'
		}))
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "assets191111"))
});
...
//		.pipe(gulp.dest(devUrl + "js/lib/"))
	...
	return gulp.src(['src/js191111/lib/jquery/jquery-1.9.1.min.js', 'src/js191111/lib/lazyLoad/jquery.lazyload.min.js','src/js191111/lib/slimscroll/jquery.slimscroll.js','src/js191111/open.js'])
	    .pipe(concat('vendor.js'))
	   	.pipe(gulp.dest(devUrl + "js191111/lib/vendor/"));
});
...
gulp.task("webpack", function() {
	return gulp.src("./src/js191111/*.js")
		.pipe(gp.debug({
			title: 'js打包:'
		}))
		.pipe(named())
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "js191111"))
});
...
gulp.task("watch", function() {
	gulp.watch("src/js191111/**/*.js", function() {
		runSequence("clean:js", "webpack", function() {
			browserSync.reload();
		});
		//		browserSync.reload();
	});
	gulp.watch("src/assets191111/**/*.*", ["assets11Move"]);
	gulp.watch("js191111/lib/**/*.*", ["libMove"]);
...
gulp.task("assets11MovePublish", function() {
	return gulp.src(devUrl + "assets191111/**/*.*")
		.pipe(gp.debug({
			title: '静态资源(图片，字体，视频，音乐等)移动:'
		}))
		.pipe(gulp.dest(publishUrl + "assets191111"))
});
//通用js库插件移动
gulp.task("libMovePublishJs", function() {
	return gulp.src(devUrl + "js191111/lib/vendor/*.js*")
		.pipe(gp.debug({
			title: 'js库移动:'
		}))
		.pipe(gp.uglify({
		...
		.pipe(gulp.dest(publishUrl + "js191111/lib/vendor"))
});
//通用js库插件css级联样式表移动
gulp.task("libMovePublishCss", function() {
	return gulp.src(devUrl + "js191111/lib/**/*.css*")
		.pipe(gp.debug({
			title: 'css库移动:'
		}))
		.pipe(gp.cleanCss({
		...
		.pipe(gulp.dest(publishUrl + "js191111/lib/"))
});
...
gulp.task("jsMin", function() {
	return gulp.src(devUrl + "js191111/*.js")
		.pipe(gp.debug({
			title: 'js压缩:'
		}))
		.pipe(babel())
		...
		.pipe(gulp.dest(publishUrl + "js191111/"))
});
//css压缩
gulp.task("cssMin", function() {
	return gulp.src(devUrl + "css191111/*.css")
		...
		.pipe(gulp.dest(publishUrl + "css191111/"))
});
//默认development环境
var knowOptions = {
	string: "env",
