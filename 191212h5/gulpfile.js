...
gulp.task("clean:js", function() {
	return gulp.src(devUrl + "js1/*.js")
		.pipe(gp.clean())
});
//清除css
gulp.task("clean:css", function() {
	return gulp.src(devUrl + "css1/*.css")
		.pipe(gp.clean())
});
//清除html
...
gulp.task('sass', function() {
	return gulp.src("./src/sass/*.scss")
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "css1"))
});

//静态等资源移动任务:图片、视频、字体等
gulp.task("assets1Move", function() {
...
//		.pipe(gulp.dest(devUrl + "js/lib/"))
	...
	return gulp.src(['src/js/lib/jquery/jquery-1.9.1.min.js', 'src/js/lib/lazyLoad/jquery.lazyload.min.js','src/js/lib/rotate/jquery.rotate.js','src/js/lib/slimscroll/jquery.slimscroll.js','src/js/open.js'])
	    .pipe(concat('vendor.js'))
	   	.pipe(gulp.dest(devUrl + "js1/lib/vendor/"));
});
...
gulp.task("watch", function() {
	...
	gulp.watch("src/assets/**/*.*", ["assets1Move"]);
	gulp.watch("js/lib/**/*.*", ["libMove"]);
...
//发布到publish
//静态等资源移动任务:图片、视频、字体等
gulp.task("assets1MovePublish", function() {
...
gulp.task("libMovePublishJs", function() {
	return gulp.src(devUrl + "js1/lib/vendor/*.js*")
		.pipe(gp.debug({
			title: 'js库移动:'
		}))
		.pipe(gp.uglify({
		...
		.pipe(gulp.dest(publishUrl + "js1/lib/vendor"))
});
//通用js库插件css级联样式表移动
gulp.task("libMovePublishCss", function() {
	return gulp.src(devUrl + "js1/lib/**/*.css*")
		.pipe(gp.debug({
			title: 'css库移动:'
		}))
		.pipe(gp.cleanCss({
		...
		.pipe(gulp.dest(publishUrl + "js1/lib/"))
});
...
gulp.task("cssMin", function() {
	return gulp.src(devUrl + "css1/*.css")
		...
		.pipe(gulp.dest(publishUrl + "css1/"))
});
//默认development环境
var knowOptions = {
	string: "env",
...
gulp.task('dev', function() {
	runSequence("clean:dist", ["sass", "libMove", "webpack", "assets1Move", "include"], "server", "watch");
});
...
gulp.task('build', function() {
	runSequence("clean:publish", ["assets1MovePublish", "libMovePublishJs", "libMovePublishCss", "change-paths", "jsMin", "cssMin"]);
});