$(function(){
	...
	if((navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i))) {
		switch(hostname){
			case '10.100.136.243':
			window.location.href = "http://test.iopen.com.cn/activity/190828.html";
			break;
			case 'www.open.com.cn':
			window.location.href = "http://m.open.com.cn/activity/190828.html";
			break;
			case 'www.iopen.com.cn':
			window.location.href = "http://m.open.com.cn/activity/190828.html";
			break;
		}
	}
	...
	$.sendReq(api.getCenter, 'get', {ac_id: 8}, function(res){
		if (res.code == 0) {
			...
			for (var i =0; i< res.data.length; i++) {
				if (i % 2 == 0) {
					var newa = [];
					newa.push(res.data[i]);
					center_data.push(newa);
				} else {
					center_data[center_data.length - 1 ].push(res.data[i]);
				}
			}
			for(var j= 0; j < center_data.length; j++) {
				$('.list').append('<li></li>')
				for(var k= 0; k <center_data[j].length; k++) {
					$('.list li').eq(j).append('<p>'+center_data[j][k]+'</p>')
				}
			}
			/*图片懒加载*/
			$.lazyImg('lazy');
	...
	$(body).on('click', '.sure', function(){
		$.setStorage('190618type', '');
		window.location.href="/users/studentSubsidies.html"
	})
...
//							showDialog('begin');
	...
	function RedB(){
		if (!RedBStatus) {
		 ...
		 var win = (parseInt($(".rain").css("width"))) - 60;
			num++;
			var hb = parseInt(Math.random() * (3 - 1) + 1);
			var Wh = parseInt(Math.random() * (70 - 30) + 110);
...
//				"-o-transform": "rotate(" + rot + ")" /* Opera */
	...
	function countDownInterval(){
		...
		if (res.step_status == 5) {
			$currentProgress.css({
				'width': parseInt(752 - 752 * (30 - countDown)/30)
			})
		} else {
			$currentProgress.css({
				'width': 752
		...
		$currentProgress.animate({'width': 0}, countDown * 1000)
