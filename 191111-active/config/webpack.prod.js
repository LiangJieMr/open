...
const getEntry = function (){
    let entry = {};
    glob.sync('./src/assets1911/js/*.js')
        .forEach(function (name,index) {
        	const key = name.split('./src/assets1911/js/')[1]
        				.split('.')[0];
            entry[key] = name;
        });
    return entry;
...
module.exports =  merge(common, {
  ...
  plugins: [
    ...
    new MiniCssExtractPlugin({
        filename: "assets1911/css/[name].css",
    ...
    new CopyWebpackPlugin([
    	{ 
    		from: 'src/assets1911/js/lib', 
    		to: path.resolve(__dirname, '../dist/assets1911/js/lib'),
    		force:true
    	}
    ])
  ]
