...
gulp.task('sass', function() {
	return gulp.src("./src/sass/*.scss")
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "css181212"))
});
...
gulp.task("assets11Move", function() {
	return gulp.src("./src/assets/**")
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "assets181212"))
});
...
gulp.task("libMove", function() {
	...
	gulp.src(["src/js/lib/**"])
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "js181212/lib/"))
		
//	return gulp.src(['src/js/lib/jquery/jquery-1.9.1.min.js', 'src/js/lib/lazyLoad/jquery.lazyload.min.js','src/js/lib/slimscroll/jquery.slimscroll.js','src/js/open.js'])
//	    .pipe(concat('vendor.js'))
//	   	.pipe(gulp.dest(devUrl + "js/lib/vendor/"));
});
...
gulp.task("webpack", function() {
	return gulp.src("./src/js/*.js")
		...
		.on('error', showErr)
		.pipe(gulp.dest(devUrl + "js181212"))
});
...
//		target: 'http://10.100.136.198:8001',
		changeOrigin: true
	})]
};
...
gulp.task("watch", function() {
	...
	gulp.watch("src/assets11/**/*.*", ["assets11Move"]);
...
gulp.task("assets11MovePublish", function() {
	return gulp.src(devUrl + "assets11/**/*.*")
		.pipe(gp.debug({
			title: '静态资源(图片，字体，视频，音乐等)移动:'
		}))
		.pipe(gulp.dest(publishUrl + "assets181212"))
});
//通用js库插件移动
gulp.task("libMovePublishJs", function() {
	return gulp.src(devUrl + "js/lib/vendor/*.js*")
		.pipe(gp.debug({
			title: 'js库移动:'
		}))
		.pipe(gp.uglify({
		...
		.pipe(gulp.dest(publishUrl + "js181212/lib/vendor"))
});
//通用js库插件css级联样式表移动
gulp.task("libMovePublishCss", function() {
	return gulp.src(devUrl + "js/lib/**/*.css*")
		.pipe(gp.debug({
			title: 'css库移动:'
		}))
		.pipe(gp.cleanCss({
		...
		.pipe(gulp.dest(publishUrl + "js181212/lib/"))
});
...
gulp.task("jsMin", function() {
	return gulp.src(devUrl + "js/*.js")
		...
		.pipe(gulp.dest(publishUrl + "js181212/"))
});
//css压缩
gulp.task("cssMin", function() {
	return gulp.src(devUrl + "css/*.css")
		...
		.pipe(gulp.dest(publishUrl + "css181212/"))
});
//默认development环境
var knowOptions = {
	string: "env",
